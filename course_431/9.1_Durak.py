...
def compare(card1, card2, trump):
    if card1[-1] == card2[-1]:
        return 'First' if values.index(card1[:len(card1)-1]) > values.index(card2[:len(card2)-1]) else 'Second'
    elif trump in (card1[-1], card2[-1]):
        return 'First' if trump == card1[-1] else 'Second'
    else:
        return 'Error'

card1, card2 = input().split()
trump = input()
print(compare(card1, card2, trump))
