...
def func(predok, potomok):
    if predok in exc_hier.keys() and potomok in exc_hier.keys():
        if predok == potomok or predok in exc_hier[potomok]:
            return True
        else:
            for i in range(len(exc_hier[potomok])):
                if func(predok, exc_hier[potomok][i]):
                    return func(predok, exc_hier[potomok][i])
                else:
                    continue
    return False
...
for i in range(int(input())):
    ...
    if ':' not in s:
        ...
        key, value = s.split(' : ')
    exc_hier.update({key : value.split()})

test = [input() for j in range(int(input()))]
